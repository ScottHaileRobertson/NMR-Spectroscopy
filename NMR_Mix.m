...
classdef NMR_Mix < handle
    ...
    methods
        function obj = NMR_Mix(area, freq, fwhm, phase)
        ...
        function componentTimeDomainSignal = calcComponentTimeDomainSignal(obj,t)
            ...
            for iComp=1:nComp
                componentTimeDomainSignal(:,iComp) = obj.area(iComp)* ...
                    exp(1i*pi/180*obj.phase(iComp) - ...
                    pi*t*obj.fwhm(iComp) + 1i*2*pi*t*obj.freq(iComp));
            end
        end
        ...
        function componentLorentzianCurves = calcComponentLorentzianCurves(obj,f)
            ...
            nFreqSamples = length(f);
            nComp = length(obj.area);
            componentLorentzianCurves = zeros(nFreqSamples,nComp);
            for iComp=1:nComp
